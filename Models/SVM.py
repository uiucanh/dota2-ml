...
def fit_model(**kwargs):
    svc = LinearSVC(random_state=1411, **kwargs)
...
def predict(model, X_test, y_test):
    ...
    print('ROC AUC score of SVC on test set: {:.2f} \n'.format(roc_auc_score(y_test, y_pred)))
...
if __name__ == '__main__':
    ...
    text_file = os.path.join(BASE_DIR, TRAINED_MODEL_DIR, dataset + 'SVCModel.txt')
    ...
    predict(svc, X_test, y_test)
    cross_validation(svc, X_matrix, y_matrix, cv=number_cv)

    print()
    print("#" * 100)
    print("Performing grid search: \n")
    clf = gridsearch(svc, X_train, y_train, cv=number_cv)

    save_model(dataset + 'SVCModel', svc)
    save_model(dataset + 'GridSVCModel', clf)
